...
@{ */
#define HOSTAPD_MAX_SSID_LEN_V01 33
//   Max ssid len is 32   add 1 byte for '\n'
#define MAX_WPA_PASS_LEN_V01 65
#define MAX_WEP_PASS_LEN_V01 33
...
#define QCMAP_CL_MAX_FILE_LEN 120
typedef struct {

	uint32_t dhcp_start_ip;
	uint32_t dhcp_end_ip;
	char     lease_time[QCMAP_CL_MAX_FILE_LEN];

}qcmap_dhcp_config_v01;  
...
typedef struct {
	...
	uint32_t netmask;
	uint8_t enable_dhcp;
	qcmap_dhcp_config_v01 dhcp_config;
	
}qcmap_lan_config_v01; 
...
typedef struct {
  ...
  char ssid[HOSTAPD_MAX_SSID_LEN_V01];
  ...
  /* Mandatory */
  uint8_t wep_key_status;
  
  /* Mandatory */
  uint8_t wep_default_key;

  /* Optional */
  uint8_t wpa_passphrase_valid;  /**< Must be set to true if wpa_passphrase is being passed */
  uint32_t wpa_passphrase_len;  /**< Must be set to # of elements in wpa_passphrase */
  char wpa_passphrase[MAX_WPA_PASS_LEN_V01];

  /* Optional */
  uint8_t wep_key0_valid;  /**< Must be set to true if wep_key0 is being passed */
  uint32_t wep_key0_len;  /**< Must be set to # of elements in wep_key0 */
  char wep_key0[MAX_WEP_PASS_LEN_V01];

  /* Optional */
  uint8_t wep_key1_valid;  /**< Must be set to true if wep_key1 is being passed */
  uint32_t wep_key1_len;  /**< Must be set to # of elements in wep_key1 */
  char wep_key1[MAX_WEP_PASS_LEN_V01];

  /* Optional */
  uint8_t wep_key2_valid;  /**< Must be set to true if wep_key2 is being passed */
  uint32_t wep_key2_len;  /**< Must be set to # of elements in wep_key2 */
  char wep_key2[MAX_WEP_PASS_LEN_V01];

  /* Optional */
  uint8_t wep_key3_valid;  /**< Must be set to true if wep_key3 is being passed */
  uint32_t wep_key3_len;  /**< Must be set to # of elements in wep_key3 */
  char wep_key3[MAX_WEP_PASS_LEN_V01];
}hostapd_basecfg; 
...
#define MAX_PROFILE_3GPP_INDEX     16
#define MIN_PROFILE_3GPP_INDEX     1
#define MAX_PROFILE_3GPP2_INDEX    56
#define MIN_PROFILE_3GPP2_INDEX    51
...
///////////add by dyy for QMI read IMSI
    /* QMI UIM service info */
    qmi_client_type           qmi_uim_handle;
    qmi_client_type           qmi_uim_notifier;
    qmi_cci_os_signal_type    qmi_uim_os_params;
//end dyy
...
boolean SetHostAPDBaseCfg(hostapd_basecfg *base_cfg, int ap_index, qmi_error_type_v01 *qmi_err_num);
boolean GetHostAPDBaseCfg(hostapd_basecfg *base_cfg, int ap_index, qmi_error_type_v01 *qmi_err_num);
boolean SetHostAPDExtCfg(hostapd_extcfg *ext_cfg, int ap_index, qmi_error_type_v01 *qmi_err_num);
boolean GetHostAPDExtCfg(hostapd_extcfg *ext_cfg, int ap_index, qmi_error_type_v01 *qmi_err_num);
boolean qcmap_cm_GetDefaultProfile(uint8 *profile_index, qmi_error_type_v01 *qmi_err_num);
boolean qcmap_cm_SetDefaultProfile(uint8 profile_index,  qmi_error_type_v01 *qmi_err_num);
boolean GetDefaultProfile(uint8 *profile_index, qmi_error_type_v01 *qmi_err_num);
boolean SetDefaultProfile(uint8 profile_index,  qmi_error_type_v01 *qmi_err_num);
boolean GetWpaSupplicantCfg(wpa_supplicant_cfg *base_cfg, int ap_index, qmi_error_type_v01 *qmi_err_num);
boolean SetWpaSupplicantCfg(wpa_supplicant_cfg *base_cfg, int ap_index, qmi_error_type_v01 *qmi_err_num);

//for profile
boolean CreateDefaultProfile();
boolean CreateDefault3gppProfile(int profilenum, simcard_type simtype, char *apn_str);
boolean ModifyDefault3gppProfile(int profilenum, simcard_type simtype, char *apn_str, int pdp_type);
boolean GetAPNInfo(int profile_index, char* apn_str, int len, char* ip_op, int* pdp_type);
boolean CreateDefaultUsrPwd();
boolean GetUsrInfo(char* usr_name, int name_len, char* usr_pwd, int pwd_len);
boolean Get_IMSI(char *imsi,qmi_error_type_v01 *qmi_err_num);
uint8 GetSimcardType(uint8 *simtype);

//for profile 
  simcard_type simtype;
  mdm_type  mdmtype;
